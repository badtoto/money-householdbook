...
namespace GMoney.control
{
    ...
    ToolboxBitmapAttribute(typeof(GMoney.control.HBarChart))]
    public partial class HBarChart : UserControl
    {
        ...
        /// </summary>
        [Browsable(true)]
        [Category("Bar Chart"), DefaultValue(24)]
        public int BarWidth
        {
        ...
        /// </summary>
        [Browsable(true)]
        [Category("Bar Chart"), DefaultValue(4)]
        public int BarsGap
        {
            get { return nBarsGap; }
            set { nBarsGap = value; }
        ...
        /// </summary>
        [Browsable(true)]
        [Category("Bar Chart"), DefaultValue(BarSizingMode.Normal)]
        ...
        /// </summary>
        [Browsable(true)]
        [Category("Bar Chart"), DefaultValue(Orientation.Vertical)]
        public Orientation BarOrientation
        {
            get { return barOrientation; }
            set { barOrientation = value; this.Items.BarOrientation = value; }
        }
...
//            tooltip.AutoPopDelay = Int32.MaxValue;
        ...
        private void DrawBars(Graphics gr, Size sizeChart)
        {
            ...
            foreach (HBarItem bar in bars)
            {
                ...
                if (Values.Visible)
                {
                    ...
                    {
                        // 2008.12.06 Gao Start -->
                        //strValue = bar.Value.ToString("F1");
                        strValue = NumericUtils.ToCurrency(bar.Value);
                        // <--End
                    }
        ...
        private void CalculatePositions(Graphics gr)
        {
            ...
            else// Orientation.Horizontal
            {
                ...
                // 2008.12.07 Gao Start -->
                if (sizingMode == BarSizingMode.AutoScale)
                {
                    // Calculate gap size
                    if (bars.Count > 0)
                    {
                        nBarsGap = 4 + (12 * this.bounds.Height) / (int)(345 * bars.Count * 7);
                        if (nBarsGap > 50) nBarsGap = 50;

                        // Calculate maximum bar size
                        nBarWidth = (int)(this.bounds.Height - rectDesc.Height - ((bars.Count + 1) * nBarsGap)) / bars.Count;
                        if (nBarWidth <= 0) nBarWidth = 24;

                        Label.FontReset();
                        Label.Visible = true;
                        if (((int)(nBarWidth - Label.Font.GetHeight(gr) - nBarsGap / 2)) > 0)
                        {
                            nBarWidth = (int)(nBarWidth - Label.Font.GetHeight(gr) - nBarsGap / 2);
                        }
                        else
                        {
                            Label.FontSetSize(Label.Font.Size * nBarWidth / 24);
                            if ((nBarWidth - Label.Font.GetHeight(gr) - nBarsGap / 2) <= 0)
                            {
                                Label.Visible = false;
                            }
                            else
                        ...
                        if (nBarWidth >= 50) nBarWidth = 50;
                        if (nBarWidth <= 0) nBarWidth = (int)Values.Font.GetHeight(gr);
                    }
                }
                // <--End

                // Where all bars start
                nStartY = (int)(this.bounds.Y + (this.bounds.Height - bars.Count * (nBarWidth + (Label.Visible ? Label.Font.GetHeight(gr) + nBarsGap / 2 : 0)) - (bars.Count + 1) * nBarsGap - rectDesc.Height) / 2);

                // don't care about negative
                foreach (HBarItem bar in bars)
                {
                    // Calculating Bar.BoundRect for each bar
                    bar.BoundRect.X = this.bounds.X + nBarsGap;
                    bar.BoundRect.Y = nStartY + i * (nBarWidth + (Label.Visible ? Label.Font.GetHeight(gr) + nBarsGap / 2 : 0)) + (i + 1) * nBarsGap + (Label.Visible ? Label.Font.GetHeight(gr) + nBarsGap / 2 : 0);
                    bar.BoundRect.Width = this.bounds.Width;
                    bar.BoundRect.Height = nBarWidth;

                    // Calculating Bar.LabelRect for each bar
                    if (Label.Visible)
                    {
                        bar.LabelRect.X = bounds.Left + nBarsGap / 2; ;
                        bar.LabelRect.Y = bar.BoundRect.Y - (Label.Font.GetHeight(gr) + nBarsGap / 2);
                        bar.LabelRect.Width = gr.MeasureString(bar.Label, Label.Font).Width;
                        bar.LabelRect.Height = Label.Font.GetHeight(gr);
                    }
                    else bar.LabelRect = RectangleF.Empty;

                    // Calculating Bar.BoundRect for each bar
                    fBoundTH = bar.BoundRect.Width - 3 * nBarsGap - (values.Visible ? (values.Mode == CValueProperty.ValueMode.Digit ? gr.MeasureString(NumericUtils.ToCurrency(bar.Value), values.Font).Width : gr.MeasureString("99.99%", values.Font).Width) : 0);
                    fBarTH = (float)((Math.Abs(bar.Value) * fBoundTH) / bars.ABSMaximum);
                    if (!(fBarTH >= 0)) fBarTH = 0;
                    bar.BarRect = new RectangleF(
                        //bar.BoundRect.X + fBoundTH - fBarTH + (values.Visible ? gr.MeasureString(values.ToString(), values.Font).Width : 0),
                        bar.BoundRect.X + nBarsGap,
                        bar.BoundRect.Y,
                        fBarTH,
                        bar.BoundRect.Height);

                    // Calculating Bar.ValueRect for each bar
                    if (Values.Visible)
                    {
                        bar.ValueRect.X = bar.BarRect.Right + nBarsGap / 2;
                        bar.ValueRect.Y = bar.BarRect.Top + (nBarWidth - values.Font.GetHeight(gr)) / 2;
                        bar.ValueRect.Width = gr.MeasureString(values.ToString(), values.Font).Width;
                        bar.ValueRect.Height = values.Font.GetHeight(gr);
                    }
                    else bar.ValueRect = RectangleF.Empty;
        ...
        private void SetCurrTooltip(HBarItem bar)
        {
            ...
            else
            {
                ...
                strCaption = String.Format("{0}\r\n{1}", NumericUtils.ToCurrency(bar.Value), strPercent);
    ...
    public class HBarItem : ICloneable
    {
        ...
        private void CreateGradientBrush()
        {
            ...
            if (parent.BarOrientation == Orientation.Vertical)
            {
            ...
            {
                this.rectGradient.X = rectBar.Left - rectBar.Width / 2;
                this.rectGradient.Y = rectBar.Top - rectBar.Height / 8;
                this.rectGradient.Width = rectBar.Width * 2;
                this.rectGradient.Height = rectBar.Height * 2;

                this.gradientCenterPoint.X = rectBar.Left + rectBar.Width / 2;
                this.gradientCenterPoint.Y = rectBar.Bottom;
            }
        ...
        void CreateGlowBrush()
        {
            ...
            {
                rectGlow = new RectangleF(rectBar.Left, rectBar.Top, rectBar.Width, rectBar.Height / 2);
